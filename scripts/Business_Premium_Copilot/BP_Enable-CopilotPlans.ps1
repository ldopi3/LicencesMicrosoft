...
# Liste des GUIDs des plans à activer (retirer de la liste des désactivés)
$toEnable = @(
...
if (-not $license) {
    ...
    # Retirer les plans à activer de la liste des désactivés
    $disabledPlans = $currentDisabled | Where-Object { $_ -notin $toEnable }

    # Construire dynamiquement l'objet licence pour éviter le bug avec DisabledPlans vide
    $licenseToAdd = @{
        SkuId = $SkuId
    }
    if ($disabledPlans.Count -gt 0) {
        $licenseToAdd.DisabledPlans = $disabledPlans
    }
    ...
    Write-Host "Les plans spécifiés ont été activés pour l'utilisateur $UserPrincipalName sur la licence $SkuId."
}